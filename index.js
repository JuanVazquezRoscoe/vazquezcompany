...
function fecharPopup() {
    document.getElementById('popupAtendimento').classList.remove('active');
...
document.getElementById('formAtendimento').addEventListener('submit', function(e) {
    ...
    // O email será enviado para juanvroscoe@gmail.com
...
function enviarParaEmail(dados) {
    ...
    // Usando FormSubmit para enviar o email
    // Não requer backend, é um serviço gratuito
    const formSubmitURL = 'https://formsubmit.co/ajax/juanvroscoe@gmail.com';
    ...
    fetch(formSubmitURL, {
        ...
        body: JSON.stringify({
            ...
            subject: `Atendimento Companhia Vazquez - ${dados.assunto}`,
            message: `
    ...
    .catch(error => {
        ...
        // Fallback: abre o cliente de email padrão
        window.location.href = `mailto:juanvroscoe@gmail.com?subject=Atendimento Companhia Vazquez - ${dados.assunto}&body=${encodeURIComponent(dados.mensagem)}`;
        mostrarNotificacao();
        fecharPopup();
        document.getElementById('formAtendimento').reset();
    })
...
function mostrarNotificacao() {
    const notificacao = document.getElementById('notificacaoSucesso');
